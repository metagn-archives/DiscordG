...
import metagn.discordg.DiscordG
import metagn.discordg.exceptions.*
import metagn.discordg.util.JSONUtil
import static metagn.discordg.util.CasingType.*
	...
	private HttpResponse<String> handleResponse(BaseRequest req, String rlUrl, HttpResponse<String> returned) {
		...
		if ((returned.headers.containsKey('X-RateLimit-Limit') &&
			...
			Thread.start {
				...
				precaution = !js.containsKey('retry_after')
				...
				if (precaution) rl.fill(js)
					...
					rl.retryTime = returned.headers['retry-after']?.getAt(0)?.toLong() ?:
				...
				while (rl.requests) {
					...
					def limit = returned.headers['x-ratelimit-limit']
					def removeAmount = limit ? limit[0].toInteger() - 1 : 1
					rl.requests.removeAll(rl.requests.sort().take(removeAmount))
