...
class PingPong {
	// Declaring it static here, so in your actual bot you can access the API object from other classes easily.
	static API api
	static main(args){
		...
		api.addListener("message create", { Event e ->
			...
			// We can get the content of that message with Message#getContent().
			if (e.data["message"].getContent().startsWith("!ping")){
				// Note that you can replace "e.data["message"]" with "e.data.message".
				e.data["message"].getChannel().sendMessage("Pong!")
